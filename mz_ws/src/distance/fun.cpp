...
#include </home/ubuntu/mz_ws/src/distance/include/fun.h>
#include <cmath>
...
request.start.pose.position.x = 0.0;//初始位置x坐标
request.start.pose.position.y = 0.0;//初始位置y坐标
...
using namespace make_plan;
int main(int argc, char** argv){
  ros::init(argc, argv, "make_planner");
  tf2_ros::Buffer buffer(ros::Duration(10));
  tf2_ros::TransformListener tf(buffer);
  ROS_INFO("Sendl");
  MoveBase MoveBase(buffer);
  size_t i =1;
  int a =0;
  ...
  ros::Subscriber sub = nh1.subscribe("int8", 1000, &Listener::callback, &listener);

  ros::Rate loop_rate(10);
  while (ros::ok())
  {
   ROS_INFO("count = %lfd    %lf",MoveBase.plannnnn_[1].x,MoveBase.plannnnn_[1].y);
    if((listener.count == 40)&&(i <(MoveBase.plannnnn_.size()-1)))
   {
     ROS_INFO("dasd");
     goal.target_pose.header.frame_id = "map";
     goal.target_pose.header.stamp = ros::Time::now();
    //goal.target_pose.pose.position =MoveBase.plannnnn_[i];
    goal.target_pose.pose.position.x = MoveBase.plannnnn_[i].x;
     goal.target_pose.pose.position.y = MoveBase.plannnnn_[i].y;
    goal.target_pose.pose.orientation.w = MoveBase.planwwwww_[i].w;//1.0;
     //goal.target_pose.pose.orientation.y = MoveBase.plannnnn_[
      ROS_INFO("Sending goal x:%lf  y:%lf",goal.target_pose.pose.position.x,goal.target_pose.pose.position.y);
     ac.sendGoal(goal);
     ac.waitForResult();
    ...
    if(i== (MoveBase.plannnnn_.size()-1))
    {
      listener.flag = 0;
       i=1;
       a=0;
...
MoveBase::MoveBase( tf2_ros::Buffer& tf):
 tf_(tf),
  ...
  //runPlanner_(false), setup_(false), p_freq_change_(false), c_freq_change_(false), new_global_plan_(false)
    ...
    starttttt_.x = x;//-0.37695;//x;//0.3
    starttttt_.y = y;//-0.364562;//y;//1

    goalllll_.x=6.75;//2.85;
    goalllll_.y = 4.0;//5.25;
    double interval=0.5;
    ...
    ROS_INFO("conntect to %s",serviceClient.getService().c_str());
    callPlanningService(serviceClient, srv,plannnnn_,planwwwww_);
...
void MoveBase::fillPathRequest(nav_msgs::GetPlan::Request &request)
  {
  request.start.header.frame_id ="map";
  request.start.pose.position.x = starttttt_.x;//初始位置x坐标
  request.start.pose.position.y = starttttt_.y;//初始位置y坐标
  request.start.pose.orientation.w = 1.0;//方向
  request.goal.header.frame_id = "map";
  request.goal.pose.position.x = goalllll_.x;//终点坐标
  request.goal.pose.position.y = goalllll_.y;
  request.goal.pose.orientation.w = 1.0;
  request.tolerance = 0.5;//如果不能到达目标，最近可到的约束
  }
  //路线规划结果回调

void MoveBase::callPlanningService(ros::ServiceClient &serviceClient, nav_msgs::GetPlan &srv,std::vector<geometry_msgs::Point> &plan,std::vector<geometry_msgs::Quaternion> &w)
  {
 double interval=0.5;
        if (serviceClient.call(srv)) {
        ...
        {
            plan.push_back(starttttt_);
            plan.push_back(goalllll_);
            //w.push_back(1.0);
            //w.push_back(1.0);
        }
        else
        {
            geometry_msgs::Point last_pose = srv.response.plan.poses[0].pose.position;
            geometry_msgs::Quaternion last_wwww = srv.response.plan.poses[0].pose.orientation;
            plan.push_back(last_pose);
            w.push_back(last_wwww);
            for(size_t i = 1; i < (srv.response.plan.poses.size()); i++)
            {
                geometry_msgs::Point pose = srv.response.plan.poses[i].pose.position;
                geometry_msgs::Quaternion wwww = srv.response.plan.poses[i].pose.orientation;
                ...
                if(dis >= interval)
                {
                    ...
                    plan.push_back(last_pose);
                    w.push_back(last_wwww);
                    ROS_INFO("x=%f,y=%f",last_pose.x,last_pose.y);
                }

            }
            plan.push_back(srv.response.plan.poses[srv.response.plan.poses.size()].pose.position);
            w.push_back(srv.response.plan.poses[srv.response.plan.poses.size()].pose.orientation);
        }
ROS_INFO("5");
...
bool MoveBase::getbaselinkpose(tf::Stamped<tf::Pose>& odom_pose,double& x, double& y, double& yaw,
                               const ros::Time& t){
...
*/
  return true;}
bool MoveBase::makeGlobalPlan(const geometry_msgs::Point &start,
                                     const geometry_msgs::Point &goal,
                                     double interval,
                                     std::vector<geometry_msgs::Point> &plan,
                                     std::vector<geometry_msgs::Quaternion> &w)
 {
  ROS_INFO("1");
  //plan.clear();
  ...
  /*double size_x = 2 * clearing_radius_, size_y = 2 * clearing_radius_;
...
*/
  ...
  ROS_INFO("3");
     ...
     start_pose.header.stamp = ros::Time();
     ...
     ROS_INFO("2");
     if(planner_->makePlan(start_pose, goal_pose, global_plan))
     {
         ROS_INFO("4");
         geometry_msgs::Point last_pose1 = global_plan[0].pose.position;
         geometry_msgs::Quaternion last_www1 = global_plan[0].pose.orientation;
         /*for(size_t i = 1; i < (global_plan.size()-1); i++)
         {
              geometry_msgs::Point pose1 = global_plan[i].pose.position;
              double dis = hypot(last_pose1.x-pose1.x, last_pose1.y-pose1.y);
              if(dis >= interval)
                {
                     last_pose1 = pose1;
                      plan.push_back(last_pose1);
                     ROS_INFO("x=%f,y=%f",last_pose1.x,last_pose1.y);
         ...
         else
         {
             ...
             geometry_msgs::Quaternion last_wwww = global_plan[0].pose.orientation;
             plan.push_back(last_pose);
             w.push_back(last_wwww);
             for(size_t i = 1; i < (global_plan.size()-1); i++)
             {
...
// bool wait_for_wake = false;
 ...
 while(n.ok()){
  ...
  // }
   ...
   /**********************************************************************************************************************************************/
   temp_goal.pose.position.x = 1.281;
   temp_goal.pose.position.y = 0.312;
   temp_goal.pose.orientation.w = 1.117;

planner_goal_ = temp_goal;
   //lock.unlock();
